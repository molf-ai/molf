...
export class CronStore {
  ...
  constructor(
    private dataPathFn: (workerId: string, workspaceId: string) => string,
  ...
  private jobsPath(workerId: string, workspaceId: string): string {
    return join(this.dataPathFn(workerId, workspaceId), "jobs.json");
  }

  private cronDir(workerId: string, workspaceId: string): string {
    return this.dataPathFn(workerId, workspaceId);
  }
  ...
  loadAll(): CronJob[] {
    const all: CronJob[] = [];
    const workersDir = join(this.pluginDir, "workers");
