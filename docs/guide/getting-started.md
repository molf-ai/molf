...
- An API key for a supported LLM provider — either `GEMINI_API_KEY` (Google Gemini) or `ANTHROPIC_API_KEY` (Anthropic Claude)
...
To use Anthropic instead of Gemini:

```bash
ANTHROPIC_API_KEY=<your-key> MOLF_LLM_PROVIDER=anthropic MOLF_LLM_MODEL=claude-sonnet-4-5-20250929 bun run dev:server
```
...
- [Configuration](/guide/configuration) — server YAML, CLI flags, environment variables, LLM providers
- [Server Overview](/server/overview) — auth tokens, LLM providers, server modules
